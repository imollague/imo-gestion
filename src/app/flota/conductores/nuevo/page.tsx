...
export default function NuevoConductorPage() {
  ...
  const handleSubmit = async () => {
    setError("")
    if (!form.nombre || !form.tipoLicencia) {
      setError("Nombre y tipo de licencia son obligatorios.")
      return
    }
    setLoading(true)
    const res = await fetch("/api/flota/conductores", {
      method: "POST",
    ...
    if (!res.ok) {
      ...
      setError(d.error || "Error al crear conductor")
      return
    }
