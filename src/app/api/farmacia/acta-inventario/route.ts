...
export async function POST(req: NextRequest) {
  ...
  const descripcion = `Acta de Inventario Farmacia Posta Rural — ${fechaStr}`
  ...
  const { publicUrl: urlOriginal } = await uploadFile(
    `firma/acta_inventario_farmacia/${doc.id}-original.pdf`,
    bufferOriginal,
    "application/pdf"
  )
  if (urlOriginal) {
  ...
  const { publicUrl: urlFirmado } = await uploadFile(
    `firma/acta_inventario_farmacia/${doc.id}-firmado.pdf`,
    bufferFirmado,
    "application/pdf"
  )
