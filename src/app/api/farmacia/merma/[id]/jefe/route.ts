...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { publicUrl: urlFirmadoFinal } = await uploadFile(
    `firma/acta_merma_farmacia/${actaId}-final.pdf`, bufferFinal, "application/pdf"
  )
    ...
    for (const item of itemsConMed) {
      ...
      await tx.movimientoFarmacia.create({
        data: {
          ...
          observacion: `[ACTA MERMA #${actaId}] Motivo: ${acta.motivo}. Stock antes: ${med.stockActual} → después: ${nuevoStock}. Autorizado por: ${auth.session.user.name}`,
  ...
  await prisma.documentoFirmado.create({
    data: {
      origen: "ACTA_MERMA_FARMACIA", origenId: actaId, descripcion,
  ...
  return NextResponse.json({
    ...
    bajaAplicada: itemsConMed.length,
  })
}
