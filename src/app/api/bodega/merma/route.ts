...
// GET — actas de merma pendientes de firma del jefe
...
// POST — registrar nueva acta de merma y firmar como responsable
export async function POST(req: NextRequest) {
  ...
  const descripcion = `Acta de Merma Bodega — ${motivoLabel[motivo] ?? motivo} — ${fechaStr}`
    ...
    data: {
      ...
      items: {
        create: items.map((i: {
          ...
          observacion: i.observacion ?? null, productoId: i.productoId,
        })),
      },
    },
  })
  ...
  const { publicUrl } = await uploadFile(
    `firma/acta_merma_bodega/${acta.id}-responsable.pdf`, buffer, "application/pdf"
  )
  ...
  await prisma.documentoFirmado.create({
    data: {
      origen: "ACTA_MERMA_BODEGA", origenId: acta.id, descripcion,
