...
export async function POST(req: NextRequest) {
  ...
  const descripcion = `Acta de Retiro Sanitario Bodega — ${causaLabel[causa] ?? causa} — ${fechaStr}`
  ...
  const { publicUrl } = await uploadFile(
    `firma/acta_retiro_bodega/${acta.id}-responsable.pdf`, buffer, "application/pdf"
  )
  ...
  await prisma.documentoFirmado.create({
    data: {
      origen: "ACTA_RETIRO_SANITARIO_BODEGA", origenId: acta.id, descripcion,
