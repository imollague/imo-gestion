...
export default function CategoriasFarmaciaPage() {
  ...
  const fetchCategorias = async () => {
    setLoading(true)
    const res = await fetch("/api/farmacia/categorias")
  ...
  return (
    <Layout titulo="Categorías — Farmacia">
                  ...
                  {editandoId === c.id ? (
                    <>
                      <input
                        ...
                        autoFocus
                      />
                      <button onClick={() => handleEditar(c.id)} className="text-green-600 text-sm hover:underline">Guardar</button>
                      <button onClick={() => setEditandoId(null)} className="text-gray-400 text-sm hover:underline">Cancelar</button>
                    </>
                  ) : (
                    <>
                      <div className="flex-1">
                        <p className="text-sm font-medium text-gray-800">{c.nombre}</p>
                        <p className="text-xs text-gray-400">{c._count.medicamentos} medicamento(s)</p>
                      </div>
                      <button
