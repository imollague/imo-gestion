...
export default function NuevaMermaBodegaPage() {
  ...
  const productosFiltrados = busqProd.length >= 1
    ? productos.filter(p =>
  ...
  const agregarItem = () => {
    ...
    if (cant > prodSeleccionado.stockActual) {
      setError(`No se puede dar de baja más que el stock actual (${prodSeleccionado.stockActual})`)
      return
    }
    setItems(prev => [...prev, {
      ...
      stockAntes: prodSeleccionado.stockActual,
      cantidad: cant,
  ...
  function buildPDF(): string {
    ...
    doc.text("ACTA DE MERMA / PÉRDIDA — BODEGA MUNICIPAL", M, y)
    y += 6
    ...
    for (const item of items) {
      ...
      doc.setTextColor(100, 0, 0); doc.setFont("helvetica", "normal"); doc.setFontSize(7)
      ...
      if (item.observacion) {
        y += 4
        doc.setTextColor(130); doc.setFontSize(6.5)
  ...
  const handleSubmit = async () => {
    ...
    const res = await fetch("/api/bodega/merma", {
      ...
      body: JSON.stringify({
        items: items.map(i => ({
          productoId: i.productoId, nombre: i.nombre, codigo: i.codigo,
    ...
    setSubmitting(false)
    if (res.ok) router.push(`/bodega?mermaId=${data.actaId}`)
    else setError(data.error || "Error al registrar el acta")
  }

  if (loading) {
    return <Layout titulo="Acta de Merma — Bodega"><p className="text-gray-500 text-sm">Cargando...</p></Layout>
  }

  return (
    <Layout titulo="Acta de Merma / Pérdida — Bodega Municipal">
      <div className="mb-6">
        <a href="/bodega" className="text-sm text-blue-600 hover:underline">&larr; Volver a Bodega</a>
      </div>

      <p className="text-sm text-gray-600 mb-6">
        Registra los productos que serán dados de baja por merma o pérdida. El acta requiere siempre la autorización del jefe.
      </p>
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 mb-5">
        <h3 className="text-sm font-medium text-gray-700 mb-3">Agregar producto a dar de baja</h3>
        <div className="flex flex-wrap gap-3 items-end">
          <div className="flex-1 min-w-48 relative">
            <label className="block text-xs text-gray-500 mb-1">Producto</label>
            <input
              type="text"
              value={prodSeleccionado ? `${prodSeleccionado.codigo} — ${prodSeleccionado.nombre}` : busqProd}
              onChange={e => { setProdSeleccionado(null); setBusqProd(e.target.value) }}
              placeholder="Buscar por nombre o código..."
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
            {!prodSeleccionado && productosFiltrados.length > 0 && (
              <div className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg mt-1 max-h-48 overflow-y-auto">
                {productosFiltrados.map(p => (
                  <button
                    key={p.id}
                    type="button"
                    disabled={yaAgregado(p.id)}
                    onClick={() => { setProdSeleccionado(p); setBusqProd("") }}
                  ...
                  >
                    ...
                    {yaAgregado(p.id) && <span className="text-xs text-blue-500 ml-1">ya agregado</span>}
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="flex-1 min-w-32">
            ...
            <input
              ...
              placeholder="Ej: lote dañado por humedad"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-400"
            />
          </div>
          <button
        ...
        {prodSeleccionado && (
          <p className="text-xs text-gray-500 mt-2">
            Stock actual: <span className="font-semibold">{prodSeleccionado.stockActual} {prodSeleccionado.unidad}</span>
          </p>
        )}
      </div>
