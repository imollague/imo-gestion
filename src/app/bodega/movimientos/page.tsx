...
const TIPOS_DOCUMENTO: Record<string, string> = {
  ...
  ACTA_DONACION: "Acta de Donacion",
...
export default function HistorialMovimientosBodegaPage() {
  ...
  const fetchMovimientos = useCallback(async (paginaActual: number) => {
    ...
    const res = await fetch(`/api/bodega/movimientos?${params.toString()}`)
  ...
  return (
    <Layout titulo="Historial de Movimientos — Bodega">
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-6 flex flex-wrap gap-4 items-end">
        ...
        <div className="flex-1 min-w-48">
          ...
          <div className="flex gap-2">
            <input type="text" placeholder="Producto, proveedor, destinatario..."
        ...
        <BotonExportar
          titulo="Historial de Movimientos - Bodega"
          subtitulo="Municipalidad de Ollagüe"
          nombreArchivo="historial-bodega"
          filas={movimientos.map((m) => ({
            fecha: formatFecha(m.fecha),
            tipo: m.tipo,
            producto: m.producto.nombre,
            codigo: m.producto.codigo,
            cantidad: `${labelCantidad(m)} ${m.producto.unidad}`,
            documento: TIPOS_DOCUMENTO[m.documento] ?? m.documento,
            numeroDocumento: m.numeroDocumento ?? "",
            proveedorDestinatario: m.tipo === TipoMovimiento.ENTRADA ? (m.proveedor ?? "") : (m.destinatario ?? ""),
            area: m.area ?? "",
          ...
          columnas={[
            ...
            { header: "Tipo", key: "tipo", ancho: 10 },
            { header: "Producto", key: "producto", ancho: 30 },
            { header: "Codigo", key: "codigo", ancho: 18 },
            { header: "Cantidad", key: "cantidad", ancho: 14 },
            { header: "Documento", key: "documento", ancho: 20 },
            { header: "N Documento", key: "numeroDocumento", ancho: 14 },
            { header: "Proveedor/Destinatario", key: "proveedorDestinatario", ancho: 25 },
            { header: "Area", key: "area", ancho: 18 },
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-red-50 rounded-lg shadow p-4 text-center">
          ...
          <p className="text-sm text-red-600">Salidas en pagina</p>
        </div>
                ...
                {movimientos.length === 0 ? (
                  ...
                  movimientos.map((m) => (
                    <tr key={m.id} className={`hover:bg-gray-50 ${m.anulado ? "opacity-50 bg-gray-50" : ""}`}>
                      ...
                      <td className="px-4 py-3">
                        <p className="font-medium text-gray-800">{m.producto.nombre}</p>
                        <p className="text-xs text-gray-400 font-mono">{m.producto.codigo}</p>
                      </td>
                      <td className="px-4 py-3 text-right font-semibold">
                        <span className={colorCantidad(m)}>
                          {labelCantidad(m)} {m.producto.unidad}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-gray-600">
                        {m.tipo !== TipoMovimiento.AJUSTE && (
                          <>
                        ...
                        {!m.anulado && !m.anulacionDeId ? (
                          <button
                            onClick={() => setMovimientoAAnular({
                              id: m.id,
                              descripcion: `${m.tipo} de ${m.cantidad} ${m.producto.unidad} - ${m.producto.nombre} (${formatFecha(m.fecha)})`
                            })}
